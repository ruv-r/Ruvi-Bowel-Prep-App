...
export default async function handler(req: any, res: any) {
  ...
  try {
    ...
    if (!apiKey) {
      console.warn("[Chat Endpoint API Router] GEMINI_API_KEY is undefined. Falling back to offline model assistant.");
...
Provide your professional response:`;
    ...
    console.error("Serverless Gemini API Error (Using Offline Fallback):", error);
    const fallbackText = generateFallbackResponse(question, cleanPrepType);
    return res.status(200).json({ text: fallbackText });
  }
}
